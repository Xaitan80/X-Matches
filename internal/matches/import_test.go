...
func TestParseCSV_WithSwedishHeaders_MapsFields(t *testing.T) {
	...
	if m.MatchNumber == "" {
		t.Errorf("match number not mapped")
	}
	if m.Weekday == "" {
		t.Errorf("weekday not mapped")
	}
	if m.DateRaw != "2025-11-08" {
		t.Errorf("date_raw = %q", m.DateRaw)
	}
	if m.TimeRaw != "14:30" {
		t.Errorf("time_raw = %q", m.TimeRaw)
	}
	if m.League == "" {
		t.Errorf("league not mapped")
	}
...
func TestParseXLSX_Basic(t *testing.T) {
	...
	if err := f.SetSheetRow(sh, "A2", &data); err != nil {
		t.Fatal(err)
	}
