...
func parseCSV(r io.Reader, ourTeam string) ([]Match, error) {
	...
	for i := 1; i < len(rows); i++ {
		if len(strings.TrimSpace(strings.Join(rows[i], ""))) == 0 {
			continue
		}
...
func normHeaders(hdr []string) map[int]string {
	...
	for i, h := range hdr {
		...
		case "sluttid":
			k = "endtimeraw"
		case "hall", "hallplats", "plats", "spelplats":
			k = "venue"
		case "stad":
			k = "city"
		case "serie", "tavling":
		...
		case "samlingplats":
			k = "gatherplace"
		case "noteringar", "notis":
			k = "notes"
		case "spelad":
			k = "played"
		case "malfor", "malhemma":
			k = "goalsfor"
		case "malmot", "malborta":
			k = "goalsagainst"
		case "toppvart", "toppskyttlag":
			k = "topscorerteam"
		case "toppmot", "toppskyttmot":
			k = "topscoreropponent"
		case "hemmalag":
			k = "hometeam"
		case "bortalag":
...
func rowToMatch(h map[int]string, row []string, ourTeam string) Match {
	...
	if r := get("result"); r != "" {
		...
		if strings.Contains(r, "-") {
			...
			gf := atoi(strings.TrimSpace(parts[0]))
			ga := int64(0)
			if len(parts) == 2 {
				ga = atoi(strings.TrimSpace(parts[1]))
			}
			if gf > 0 || ga > 0 {
				m.GoalsFor, m.GoalsAgainst, m.Played = gf, ga, true
			}
		}
	}
